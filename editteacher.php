...
<div class="c-eighteen" >
    ...
    <div class="right">
        <h3 style='color:goldenrod;'>editteacher.php</h3>
<?php
$host="localhost";
$username="root";
$password="";
...
?>
<!-- html starts -->
        <form class="editform"name="frmteacherupdate" action="updateteacher.php" method="POST" enctype="multipart/form-data">
            <div>
                <label for="txtid">id</label>
                <input type="visible" name="txtid" value="<?php echo $row['id'];?>"readonly>
            </div>
            <div>
                <label for="txtteacher">name</label>
                <input type="text" name="txtteachername" id="txtteacher" value="<?php echo $row['name'];?>">
            </div>
            <div>
                <label for="txtdescription">description</label>
                <input type="text" name="txtdescription" id="txtdescription" value="<?php echo $row['description'];?>">
            </div>
            <div>
                <label for="txtfile">image</label>
                <img style="width:50px; height=40px;" src="<?php echo $row['image'];?>">
                <!-- new image select and preview by using below given code -->
                <input type="file" name="newimgupldinpt" id="inptuploadr">
        ...
        $deluser="DELETE FROM teacher WHERE id=$id";
        $result=mysqli_query($cn,$deluser) or die('could not delete user'.mysqli_error($cn));
        if($result>0){
            $_SESSION['deletesuccessfully']='teacher data delete successfully';
